function [Ct] = win_always_ch(dyn, C0, rho, show_plot, verbose)
...
% Automatic Control, dx.doi.org/10.1109/TAC.2017.2672859
  ...
  if length(rho) == 1
    rho = rho * ones(dyn{1}.nx,1);
  end

  rho_ball = Polyhedron('A', [eye(dyn{1}.nx); -eye(dyn{1}.nx)], ...
                        'b', repmat(rho,2,1));

  C = Polyhedron('A', zeros(1,dyn{1}.nx), 'b', 1);
  Ct = C0;
  iter = 0;
